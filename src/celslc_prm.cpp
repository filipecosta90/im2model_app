      ...
      )
    {
      ...
      if(  _flag_io_ap_pipe_out  ){
        ...
        if( enable_ssc ){
          ...
          std::stringstream args_stream_ssc_dummy;
          create_bin_args( args_stream_ssc_dummy, enable_ssc );

          boost::process::child c(
              // command
            args_stream_ssc_dummy.str(),
            boost::process::start_dir= base_bin_start_dir_path,
          ...
          for ( int slice_id = 1; slice_id <= nz_simulated_partitions; slice_id++ ){
            ...
            ssc_stream << args_stream_ssc.str() << " -ssc " << slice_id;
          ...
          while (!ssc_queue.empty()){
            ...
            for (auto it = ssc_queue.begin(); it != ssc_queue.end(); ){
              ...
              if (!c.running()){
                ...
                if( _flag_logger ){
                  std::stringstream message;
                  message << "Slice # "<<  it->first << " _child_exit_code value: " << std::boolalpha << _child_exit_code;
                  BOOST_LOG_FUNCTION();  logger->logEvent( ApplicationLog::notification , message.str() );
                }
              }
              else{
