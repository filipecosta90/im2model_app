      ...
      for (int defocus = 0; defocus < defocus_samples ; defocus ++ ){
        try{
          ...
          switch ( normalization_mode ) {
            ...
            const double alpha = brightest_pixels_intensity_level/interval;
            const double beta = -alpha * darkest_pixel_intensity_level;
      ...
      for (int thickness = 0; thickness < slice_samples; thickness ++ ){
        for (int defocus = 0; defocus < defocus_samples ; defocus ++ ){
          try{
            ...
            double min, max;
            cv::minMaxLoc(normalized_simulated_image, &min, &max);
            double interval = ( global_max - global_min );
            const double alpha = brightest_pixels_intensity_level/interval;
            const double beta = -alpha * darkest_pixel_intensity_level;
            normalized_simulated_image.convertTo(normalized_simulated_image, normalized_simulated_image.type() , alpha, beta );
