...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
  ...
  _sim_output_thread = new QThread(this);
  sim_output_updater = new GuiSimOutUpdater( _core_td_map );
  sim_output_updater->moveToThread( _sim_output_thread );

  // will only start thread when needed
  connect(sim_output_updater, SIGNAL(TDMap_request()), _sim_output_thread, SLOT(start()));
  connect(_sim_output_thread, SIGNAL(started()), sim_output_updater, SLOT(newTDMapSim()));
  connect(sim_output_updater, SIGNAL(TDMap_sucess()), this, SLOT(update_from_TDMap_sucess()));
  connect(sim_output_updater, SIGNAL(TDMap_failure()), this, SLOT(update_from_TDMap_failure()));
  // will quit thread after work done
  connect(sim_output_updater, SIGNAL(finished()), _sim_output_thread, SLOT(quit()), Qt::DirectConnection);
...
MainWindow::~MainWindow(){
  ...
  sim_output_updater->abort();
  _sim_output_thread->wait();
  qDebug()<<"Deleting thread and worker in Thread "<<this->QObject::thread()->currentThreadId();
  delete _sim_output_thread;
  delete sim_output_updater;
...
void MainWindow::on_qpush_run_tdmap_clicked(){
  ...
  sim_output_updater->requestTDMap();

  // sim_output_updater->newTDMapSim( _core_td_map );
  /*
#ifndef QT_NO_CURSOR
